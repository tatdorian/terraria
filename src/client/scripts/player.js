...
class Player {
  ...
  pickUp(item) {
    ...
    const sameItemIndex = this.inventory.findIndex(
      (slot) =>
        slot !== null && slot.name === item.name && slot.quantity < 99 // Limite de stack à 99
    );
  ...
  handleInput(e, isKeyDown) {
    switch (e.key.toLowerCase()) {
      ...
      case 'c':
        if (isKeyDown) {
          // Crafter une pioche
          console.log('Touche C pressée pour crafter');
          this.craftItems();
        }
        break;
    }
  ...
  craftPickaxe() {
    ...
    this.inventory.forEach((slot) => {
      if (slot && slot.name === 'stone') brickCount += slot.quantity;
    });

    if (brickCount < 5) {
      console.log('Not enough bricks to craft a pickaxe.');
      return;
    }

    // Remove 5 bricks
    let bricksToRemove = 5;
    for (let i = 0; i < this.inventory.length; i++) {
      const slot = this.inventory[i];
      if (slot && slot.name === 'stone') {
        const canRemove = Math.min(slot.quantity, bricksToRemove);
        slot.quantity -= canRemove;
        bricksToRemove -= canRemove;
        if (slot.quantity <= 0) this.inventory[i] = null;
        if (bricksToRemove <= 0) break;
      }
    }
    ...
    if (emptySlot !== -1) {
      ...
      console.log('Crafted a pickaxe!');
    } else {
      console.log('Inventory is full, cannot add pickaxe.');
    }
  }
  ...
  update(world) {
    ...
    if (
      !(this.velocity.x < 0 && (topLeft || bottomLeft)) &&
    ...
    let isOnGround =
      world.isSolid(this.x + playerWidth, feetY) ||
      world.isSolid(this.x + playerWidth * 2 - 10, feetY);

    // Collision avec le plafond
    let hitHead =
      world.isSolid(this.x + playerWidth, headY + this.velocity.y) ||
    ...
    isOnGround =
      world.isSolid(this.x + playerWidth, this.y + 100) ||
  ...
  // Ajout d'une nouvelle méthode pour le craftage
  craftItems() {
    if (this.inventoryOpen) {
      // Vérifier si on peut crafter une pioche
      let stoneCount = 0;
      this.inventory.forEach((slot) => {
        if (slot && slot.name === 'stone') {
          stoneCount += slot.quantity || 1;
        }
      });

      if (stoneCount >= 5) {
        // Supprimer 5 pierres de l'inventaire
        let stonesToRemove = 5;
        for (let i = 0; i < this.inventory.length; i++) {
          const slot = this.inventory[i];
          if (slot && slot.name === 'stone') {
            const removeCount = Math.min(slot.quantity || 1, stonesToRemove);
        ...
        // Ajouter la pioche à l'inventaire
        ...
        if (emptySlot !== -1) {
          ...
          const pickaxe = {
            ...
            item_type: 'tool',
            quantity: 1,
            texture: TextureLoader.get('player', 'idle'), // Temporairement utiliser une texture existante
          };
          this.inventory[emptySlot] = pickaxe;
          console.log('Pioche craftée avec succès!');
          return true;
        } else {
          console.log('Inventaire plein, impossible de crafter la pioche');
          return false;
        }
      } else {
        console.log('Pas assez de pierres pour crafter une pioche (5 requises)');
        return false;
      }
    }
    return false;
  ...
  // Ajout d'une méthode pour ramasser manuellement (touche R)
  tryPickupNearbyItems(world) {
    const range = 100; // Distance de ramassage en pixels
    let itemsPickedUp = 0;

    world.droppedItems = world.droppedItems.filter((drop) => {
      const dx = Math.abs(this.x + 50 - drop.x);
      const dy = Math.abs(this.y + 50 - drop.y);

      if (dx < range && dy < range) {
        if (this.pickUp(drop.item)) {
          itemsPickedUp++;
          return false; // Retirer l'item de la liste
        }
      }
      return true; // Garder l'item dans la liste
    });

    if (itemsPickedUp > 0) {
      console.log(`${itemsPickedUp} objet(s) ramassé(s)`);
      return true;
    }
    return false;
  }
